...
with eval as (select * from {{ ref('stg_sqlserver__evaluacion_desempeno') }})
...
from eval e
left join {{ ref('dim_empleado') }} de on e.id_empleado_nk = de.id_empleado_nk and de.es_actual = true