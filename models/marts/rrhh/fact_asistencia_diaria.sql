...
with asis as (select * from {{ ref('stg_sqlserver__asistencia') }}),
     asign as (select * from {{ ref('stg_sqlserver__asignacion_turnos') }})

select
    cast(to_char(a.hora_entrada_real, 'YYYYMMDD') as integer) as id_fecha_sk,
    de.id_empleado_sk,
    dt.id_turno_sk,
...
left join {{ ref('dim_empleado') }} de on a.id_empleado_nk = de.id_empleado_nk and de.es_actual = true
left join {{ ref('dim_turno') }} dt on asg.id_turno_nk = dt.id_turno_nk